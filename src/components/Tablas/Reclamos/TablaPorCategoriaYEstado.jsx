...
function descendingComparator(a, b, orderBy) {
  ...
  const columnas = ["Categoría", "Total"];
...
const TablaPorCategoriaYEstado = () => {
  const [order, setOrder] = React.useState('asc');
    ...
    React.useEffect(() => {
      ...
      resultSearch[0].forEach((obj) => {
        const { id_categoria, cantidad } = obj;

        if (!categoriasSumadas[id_categoria]) {
          categoriasSumadas[id_categoria] = { ...obj }; // Clonar el objeto para evitar referencias directas
        } else {
          categoriasSumadas[id_categoria].cantidad += cantidad;
        }
      });
...
export default TablaPorCategoriaYEstado