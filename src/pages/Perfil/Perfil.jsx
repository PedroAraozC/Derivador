...
const Perfil = () => {
  ...
  const validarEmail = async () => {
    if (usuario.validado == 1) {
      return Swal.fire({
        text: "Su email ya está validado!",
        confirmButtonColor: "#1F89F6",
      });
    }

    try {
      await cdigitalApi.post(`/usuarios/email`, {
        email_persona: user.email_persona,
      });
      abrirModal();
    } catch (error) {
      ...
      <div className="d-flex flex-column p-2 mt-3 contenedorPerfil">
        ...
        {isEditing ? (
          <div className="px-2">
            ...
            <p>{usuario.fecha_nacimiento_persona.split("T")[0]}</p>
            <p className="datoUsuario">Localidad:</p>{" "}
            <p> {usuario.localidad_persona}</p>
            <p className="datoUsuario">Domicilio:</p>{" "}
            <p> {usuario.domicilio_persona}</p>
          ...
          {isEditing ? (
            <>
              ...
              <Button onClick={validarEmail} variant="outlined">
                <MarkEmailReadIcon className="me-2" color="primary" /> Validar
                email
              </Button>
            </>
          ) : (
